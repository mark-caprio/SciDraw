...
Canvas::usage="Canvas[{x,y}] represents a point in canvas coordinates (printer's points from lower left corner of main canvas region) as an argument to FigAnchor.";  (* DEBUGGING: Symbol Canvas is entirely blocked from namespace (not just shadowed!) by special symbol System`Canvas introduced in Mathematica 12.2 *)
...
CanvasCoordinates::usage="CanvasCoordinates[{x,y}] represents a point in canvas coordinates (printer's points from lower left corner of main canvas region) as an argument to FigAnchor.";  (* was Canvas *)
ScaledCoordinates::usage="ScaledCoordinates[{x,y}] represents a point in scaled coordinates (fractional coordinates across current panel) as an argument to FigAnchor.";
